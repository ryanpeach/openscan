...
def saveImage(img, filepath="", name=""):
    ...
    global saveN
    if filepath == "":
        filepath = os.path.dirname(__file__) + "/saves/"
    if name == "":
        name = "save{}.png".format(str(saveN))
        saveN += 1
    cv2.imwrite(filepath+name,img)
...
def imageTest(img):
    ...
    ref = getRef(fp,depth)
    preview = fixPerspective(img,sort,ref)
    preview = cropImage(preview,.04*preview.shape[0])
    preview = filterOut(preview)
    out = drawBorder(img,sort)
    return (out,preview)
